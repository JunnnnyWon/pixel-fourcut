...
def _is_preferred_paper(name: str | None, width: int | None, height: int | None) -> bool:
    paper_name = (name or "").lower()
    if any(token in paper_name for token in PREFERRED_PAPER_NAME_TOKENS):
        return True
    ...
    return 380 <= short_edge <= 410 and 560 <= long_edge <= 610
...
def get_printer_capabilities(printer_name: str) -> dict:
    if platform.system() != "Windows":
        return {"printer_name": printer_name, "paper_sizes": [], "preferred_paper": None}
...
$doc.PrinterSettings.PrinterName = '{ps_printer}'
if (-not $doc.PrinterSettings.IsValid) {{
  throw "Printer not found: {ps_printer}"
}}
$paperSizes = @()
...
[ordered]@{{
  printer_name = '{ps_printer}'
  default_paper_name = $doc.DefaultPageSettings.PaperSize.PaperName
...
"""
    ...
    preferred = next(
        (
            paper
            for paper in data.get("paper_sizes", [])
            if _is_preferred_paper(paper.get("name"), paper.get("width"), paper.get("height"))
        ),
        None,
    )
    data["preferred_paper"] = preferred
    return data
...
$paper = $doc.PrinterSettings.PaperSizes | Where-Object {{
  ($_.PaperName -match 'Postcard|4x6|4 x 6|100 x 148|100x148') -or
  (([Math]::Min($_.Width, $_.Height) -ge 380) -and ([Math]::Min($_.Width, $_.Height) -le 410) -and ([Math]::Max($_.Width, $_.Height) -ge 560) -and ([Math]::Max($_.Width, $_.Height) -le 610))
...
if ($paper) {{
  $result['paper_name'] = $paper.PaperName
  $result['paper_width'] = $paper.Width
  $result['paper_height'] = $paper.Height
}} else {{
  $result['paper_name'] = $doc.DefaultPageSettings.PaperSize.PaperName
}}
