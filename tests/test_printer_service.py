...
class PrinterServiceTests(unittest.TestCase):
    ...
    def test_get_printer_capabilities_marks_preferred_paper(self):
        payload = json.dumps(
            {
                "printer_name": "SELPHY-LEFT",
                "default_paper_name": "A4",
                "paper_sizes": [
                    {"name": "A4", "width": 827, "height": 1169, "kind": "A4"},
                    {"name": "Postcard", "width": 394, "height": 583, "kind": "Custom"},
                ],
            }
        )
        with patch("backend.printer_service.platform.system", return_value="Windows"), \
             patch("backend.printer_service._run_powershell", return_value=payload):
            capabilities = printer_service.get_printer_capabilities("SELPHY-LEFT")

        self.assertEqual(capabilities["preferred_paper"]["name"], "Postcard")
