...
def slide_summary(prs):
    ...
    add_bullets(s, Inches(0.8), Inches(4.4), Inches(11.8), Inches(2.1), [
        "로컬 PC 없이도 FLUX.2 기반 AI 사진관을 축제 현장에서 무중단으로 굴릴 수 있었고, On-Demand 21h 26m / 36,779원으로 단발성 행사 진입 장벽이 낮았습니다.",
...
def slide_project(prs):
    ...
    add_bullets(s, Inches(0.5), Inches(2.2), Inches(7.5), Inches(4.5), [
        "Canon EOS 60D로 손님을 촬영합니다.",
        "촬영본을 ComfyUI + FLUX.2 기반 AI 파이프라인이 자동으로 변환합니다.",
...
def slide_pipeline(prs):
    ...
    add_header(s, "기술 파이프라인",
               "Canon EOS 60D → 워치 폴더 → ComfyUI(FLUX.2) → SELPHY CP1500.",
               page=5)
    ...
    add_bullets(s, Inches(0.5), Inches(5.1), Inches(12.3), Inches(2.0), [
        "FLUX.2 기반 워크플로우라 VRAM 여유가 중요했습니다. A40 48G로 충분히 여유 있게 동작했습니다.",
...
def slide_runyour_usage(prs):
    ...
    add_bullets(s, Inches(0.8), Inches(5.95), Inches(11.7), Inches(0.9), [
        "FLUX.2 워크플로우의 VRAM 여유를 우선했습니다. (모델 + LoRA + 워크플로우 동시 적재)",
...
def slide_good(prs):
    ...
    cards = [
        ("진입 장벽이 낮음",
         "GPU 카드 구매·PC 신규 구성 없이 축제 한 번을 위한 단발성 환경을 구성할 수 있었습니다. 학생 팀 예산 범위에서 FLUX.2 워크플로우를 굴릴 수 있는 선택지였습니다."),
        ("48G VRAM 여유",
         "FLUX.2 + 보조 모델 + ComfyUI 워크플로우를 한 인스턴스에 적재한 상태로 OOM 없이 운영했습니다. 워크플로우 변경 시에도 메모리 압박은 없었습니다."),
        ("On-Demand 비용 예측 가능성",
...
def slide_bad(prs):
    ...
    cards = [
        ...
        ("워크플로우 셋업은 사용자 몫 (추천 사항)",
         "ComfyUI·FLUX.2 환경 구성은 사용자 몫이라는 점은 클라우드 GPU 특성상 당연하다고 생각합니다. 다만 RunYour가 제공한 ComfyUI 템플릿은 구버전이라 포트 충돌 등으로 그대로 쓰기 어려웠고, 결국 Ubuntu에 수동 설치로 우회했습니다. \"학생·소규모 프로젝트용 최신 ComfyUI 템플릿\"이 갱신되면 진입이 한결 부드러울 것 같습니다."),
        ("템플릿/이미지 생태계가 비교적 단출함",
         "RunPod·Vast.ai와 비교하면 사전 구성된 템플릿(PyTorch·ComfyUI·SD WebUI 등) 종류가 적은 편이었습니다. 한국어 환경과 결제 편의는 큰 강점이라 \"국내 사용자용 큐레이션된 템플릿 카탈로그\"가 늘어나면 그 강점이 더 살 것 같습니다."),
        ("스토리지 옵션의 유연성",
         "현재는 인스턴스 디스크 중심이라, 인스턴스를 끄면 데이터를 매번 다시 올려야 했습니다. RunPod의 Network Volume처럼 인스턴스와 분리된 영속 볼륨 옵션이 있으면 모델·워크플로우 재사용이 훨씬 편할 것 같습니다."),
    ]
    top = Inches(1.85)
    cols = 3
    rows = 2
