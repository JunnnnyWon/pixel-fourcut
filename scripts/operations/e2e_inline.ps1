...
$p = Start-Process -PassThru -FilePath (Join-Path $repo '.venv\Scripts\python.exe') -ArgumentList '-m','uvicorn','backend.main:app','--host','127.0.0.1','--port','8000' -WorkingDirectory $repo -RedirectStandardOutput $outLog -RedirectStandardError $errLog
for ($i = 0; $i -lt 20; $i++) {
  Start-Sleep -Milliseconds 500
  try {
    $null = Invoke-RestMethod -Method Get -Uri "$base/api/comfyui/status"
    break
  } catch {
